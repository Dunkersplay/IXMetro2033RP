...
function PANEL:Init()
	...
	self.infolabel:SetText("Coloca el ratón encima del icono de un objeto para obtener más detalles.")
...
function PANEL:SetItem(name, icon, desc, req, results, skills, blueprint, workstation, tools, entity)
	...
	self.spawnicon:SetHelixTooltip(function(tooltip)
		...
		if self.tools then
			...
			for _, v in pairs(self.tools) do
			rtools[#rtools + 1] = ix.item.list[v].name
			end
